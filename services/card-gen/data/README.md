...
data/
└── cards/
    ├── basic.yaml          # 「基本セット」（カテゴリ横断のおすすめ10選）
    ├── sns.yaml            # 「SNSミーム」
    ├── tv.yaml             # 「テレビミーム」
    └── anime_manga.yaml    # 「漫画・アニメミーム」
```

1 ファイル＝1 セット。ファイル名はセット ID と一致させる規約（必須ではないが推奨）。
カードの `category` 値（SNS / テレビ / 漫画・アニメ）は所属ファイルと揃える。
`basic.yaml` のみカテゴリ横断のキュレーション用で、他ファイルとカード内容が重複してもよい（ID は全体ユニーク必須）。
